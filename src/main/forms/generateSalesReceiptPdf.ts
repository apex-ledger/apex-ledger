...
export async function generateSalesReceiptPdf(receipt: SalesReceipt, customer: Contact, company: CompanyInfo, depositAccountName: string): Promise<Uint8Array> {
  ...
  function newPage() {
    ...
    if (pageNumber === 1) {
      y = drawDocumentHeading(page, { title: 'SALES RECEIPT', legalName: company.legalName, logo, font, boldFont, titleColor: BRAND_900, nameColor: TEXT_DARK, ruleColor: BRAND_900 });
    } else {
