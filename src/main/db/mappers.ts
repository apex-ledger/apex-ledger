...
export function mapInvoiceRow(row: Selectable<InvoiceTable>, lines: InvoiceLine[]): Invoice {
  return {
    ...
    writtenOffCents: (row as { writtenOffCents?: number }).writtenOffCents ?? 0,
    writeOffJournalEntryId: (row as { writeOffJournalEntryId?: number | null }).writeOffJournalEntryId ?? null,
    balanceDueCents: Math.max(0, row.totalCents - (row.paidCents ?? (row.status === 'paid' ? row.totalCents : 0)) - ((row as { writtenOffCents?: number }).writtenOffCents ?? 0)),
    lines,
    foreignCurrency: asForeignCurrency(row.foreignCurrency),
    foreignAmountCents: row.foreignAmountCents,
    exchangeRate: row.exchangeRate,
